...
EMAuxiliary <- function(formula, data, id,
                       ...
                       ordinal = NULL,
                       nominal = NULL,
                       burn = 5000, iter = 10000, chains = 2, seed = 12345) {
  ...
  if (.bw_has_markers(ftxt) && !.__blimpwrap_markers_warned) {
    message("[blimp_wrap] Note: within()/between() (and w()/b()) are supported for convenience, ",
  ...
  if (!is.null(aux) && length(aux)) {
    ...
    for (a in aux) {
      if (a %in% names(data2)) {
        xa <- data2[[a]]
        if (is.numeric(xa) && length(unique(na.omit(xa))) >= 3) {
    ...
    if (length(aux_scaled)) {
      message("[blimp_wrap] Standardized continuous auxiliaries: ",
    ...
    if (length(sd_ok) >= 1) {
      ...
      if (trigger) {
        ...
        addendum <- paste0(
          ...
          "Focal SDs:\n", sd_tbl, "\n\n",
          "Tip: Consider z-scaling y and the largest-SD predictors BEFORE calling blimp_wrap().\n"
        )
      }
    }
