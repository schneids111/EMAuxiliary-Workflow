...
prompt_var <- "daily_prompt"  # OR "prompt_num" if using global counter
# ----------------------
...
# Determine expected number of prompts per group
expected_prompts <- ema_df %>%
  summarise(n_prom = max(.data[[prompt_var]], na.rm = TRUE)) %>%
  pull(n_prom)

# Mark observed rows
ema_df <- ema_df %>% mutate(missed_prompt = 0L)

# Insert missing rows
ema_df <- ema_df %>%
  group_by(across(all_of(group_vars))) %>%
  complete(!!sym(prompt_var) := seq_len(expected_prompts)) %>%
  ungroup()
...
if (!is.na(day_var) && day_var %in% names(ema_df)) {
  ema_df <- ema_df %>%
    mutate(
      prompt_num = ( .data[[day_var]] - 1L ) * expected_prompts + .data[[prompt_var]]
    )
}

# Identify missed prompts
ema_df <- ema_df %>%
  mutate(
    missed_prompt = if_else(
      is.na(mood) & is.na(stress),
      1L, 0L
    )
  )
