...
Each example is a stand-alone demonstration of common modeling issues. They are not presented to be run in sequence. 
...
| **2**   | Cross-level interaction with latent mean centering (no auxiliaries)|
| **3**   | Same cross-level interaction, with auxiliary variables added |
| **4**   | Specification of a binary outcome and ordinal and nominal auxiliary variables|
| **5**   | Poor model convergence|
...
- **Load EMAuxiliary function from GitHub**: source("https://raw.githubusercontent.com/schneids111/EMAuxiliary-Workflow/main/EMAuxiliary/EMAuxiliary.R")
...
# 3. Example 1 – Cross-level interaction (no auxiliaries)
...
# 4. Example 2 – Cross-level interaction with latent mean centering (no auxiliaries)
...
y ~ x * x.mean + (1 + x | id)
```
`x` = latent within-person component (activated with center_group = "x")
...
# 5. Example 3. Cross-level interaction with latent mean centering  and continuous auxiliaries

**Goal:** Estimate a model with continous auxiliary variables to improve missing-data handling.  
...
Do I **need** to declare ordinal/nominal?
- Yes when a variable is binary/ordinal/nominal and is included as an outcome in the Blimp model—either the focal outcome or as an auxiliary being jointly modeled for missing-data handling.
- Often **not necessary** when the same variable is used only as a predictor (e.g., Likert 1–5 used as a covariate). In that case, treating it as continuous is typically acceptable and mirrors common practice in multilevel models.
- **When in doubt**: prioritize correct specification for outcomes and auxiliaries, and keep predictors simple unless you have a reason not to.
- Declaring types is about how Blimp models the variable’s distribution in the joint model—not about policing measurement theory.

**Model specification**

```text
y_bin ~ x + x.mean + (1 + x | id)
