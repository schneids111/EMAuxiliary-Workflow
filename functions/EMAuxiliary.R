...
.bw_is_level2_var <- function(v, id, data_work) {
  if (!v %in% names(data_work)) return(FALSE)
  sds <- tapply(data_work[[v]], data_work[[id]], function(z) stats::sd(z, na.rm = TRUE))
  all(is.na(sds) | sds < 1e-8)
}
.bw_icc <- function(v, id, data_work) {
  x <- data_work[[v]]
  grp <- data_work[[id]]
...
EMAuxiliary <- function(formula, data, id,
  ...
  # Extract fixed-effect bases
  fixed_form0 <- lme4::nobars(formula)
  tl_raw0 <- attr(terms(fixed_form0), "term.labels")
  bases_fixed <- unique(
    .bw_strip_markers(unlist(strsplit(tl_raw0, ":", fixed = TRUE)))
  )
  bases_fixed <- bases_fixed[nzchar(bases_fixed)]
  
  # Extract random-effect bases
  fb0 <- lme4::findbars(formula)
  bases_random <- character(0)
  if (length(fb0)) {
    bases_random <- unique(unlist(lapply(fb0, function(b) {
      left <- deparse(b[[2]])
  ...
  # Remove latent-mean variables (x.mean) from validation
  intended_vars_clean <- intended_vars[!grepl("\\.mean$", intended_vars)]
  
  # Catch missing variables early
  missing_vars <- setdiff(intended_vars_clean, names(data))
  if (length(missing_vars) > 0) {
    stop(
      "[EMAuxiliary] ERROR: The following variable(s) were referenced\n",
      "in the formula or arguments but do NOT exist in the dataset:\n  ",
      paste(missing_vars, collapse = ", "),
      "\n\nPlease fix variable names or remove them.",
      call. = FALSE
    )
  }
  
  ###########################################################################
  ### NEW BLOCK 2 — Make a safe, pruned working copy of the dataset      ###
  ###########################################################################
  
  needed <- intersect(intended_vars_clean, names(data))
  data_work <- data[, needed, drop = FALSE]
  
  ###########################################################################
  ### ORIGINAL CODE CONTINUES, but now using data_work everywhere        ###
  ###########################################################################
  ...
  base_raw  <- unique(.bw_strip_markers(
    unique(unlist(strsplit(tl_raw, ":", fixed = TRUE)))
  ))
  ...
  if (length(rand_slopes_raw)) {
    if (any(grepl("\\.mean\\b", rand_slopes_raw))) {
      stop("Random slopes cannot include latent means (*.mean).", call. = FALSE)
    }
  }
  ...
  if (length(cg))            center_lines <- c(center_lines, paste0("groupmean = ", paste(cg, collapse = " "), ";"))
  if (length(center_grand))  center_lines <- c(center_lines, paste0("grandmean = ", paste(center_grand, collapse = " "), ";"))
  ...
  # Identify pure L2 variables in pruned dataset
  pure_L2_obs <- base_raw[vapply(base_raw, function(v)
    .bw_is_level2_var(v, id, data_work), logical(1))]
  ...
  ###########################################################################
  ### AUXILIARY MODEL construction (using data_work throughout)           ###
  ###########################################################################
  ...
  if (!is.null(aux) && length(aux)) {
    ...
    aux_missing <- setdiff(aux, names(data_work))
    if (length(aux_missing)) {
      stop(
        "[EMAuxiliary] ERROR: Auxiliary variable(s) not found in data: ",
    ...
    for (a in aux) {
      ...
      a_is_L2 <- .bw_is_level2_var(a, id, data_work)
      a_icc   <- if (!a_is_L2) .bw_icc(a, id, data_work) else 1
      ...
      if (isTRUE(a_is_L2)) {
        ...
        raw_ok <- intersect(main_bases, names(data_work))
  ...
  obs_vars <- obs_vars[obs_vars %in% names(data_work)]
  ...
  blimp_code <- paste(
    ...
    paste0("CLUSTERID: ", id, ";"),
    if (!is.null(ordinal) && length(ordinal))
      paste0("ORDINAL: ", paste(ordinal, collapse = " "), ";") else NULL,
    if (!is.null(nominal) && length(nominal))
      paste0("NOMINAL: ", paste(nominal, collapse = " "), ";") else NULL,
  ...
  ###########################################################################
  ### Run rblimp with pruned dataset (data_work)                         ###
  ###########################################################################
  ...
  ###########################################################################
  ### PSR diagnostics — now using data_work consistently                  ###
  ###########################################################################
  ...
  if (is.finite(psr) && psr > 1.05) {
    ...
    sds <- vapply(focal_cols, function(v) {
      if (!v %in% names(data_work) || !is.numeric(data_work[[v]])) return(NA_real_)
      stats::sd(data_work[[v]], na.rm = TRUE)
    }, numeric(1))
    ...
    if (length(sd_ok) >= 1) {
      ...
      trigger <- (length(sd_ok) >= 2 && ratio > ratio_thresh) ||
        any(sd_ok > abs_thresh)
  ...
  if (!is.null(aux_model_arg) && exists("used_aux_route") &&
      used_aux_route == "inlined-into-model") {
